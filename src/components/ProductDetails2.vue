    ...
    <div class="container px-5 py-24 mx-auto">
      <div class="lg:w-4/5 mx-auto flex flex-wrap">
        ...
        <img alt="" class="lg:w-1/2 w-full object-cover object-center rounded border border-gray-200"
          :src="`${store.currentProduct?.data?.product_image}`">
        <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
          <h2 class="text-sm title-font text-gray-500 tracking-widest">REF205 </h2>
          <h1 class="text-black text-3xl title-font font-medium mb-1">{{ store.currentProduct?.data?.name }}</h1>

          <p class="leading-relaxed">{{ store.currentProduct?.data?.description }}</p>
          <div>
            <h1 class="text-black text-3xl title-font font-medium mb-1">Catégorie:</h1>
             <span>{{ store.currentProduct?.data?.category.name }}</span> 

            <h1 class="text-black text-3xl title-font font-medium mb-1">Matériau(x) utilisé(s):</h1>
            <ul v-for=" material in store.currentProduct?.data?.materials">
              <li> {{ material.name }}</li>
              <!-- <li>Bois</li>
              <li>Verre</li>
              <li>Cuivre</li> -->
          ...
          <div class="flex">
            <span class="title-font font-medium text-2xl text-black">Prix: {{ store.currentProduct?.data?.price }} €</span>
          </div>
...
const  currentProduct  = toRef(store, 'currentProduct')
  
onMounted(()=> {
    store.getProductById(route.params.id);
    console.log('current:',currentProduct.value)
   // console.log(currentProduct.data, 'test2');
})
